...
class TestSwitchToOc:
    ...
    def test_saves_omo_entry_to_txt(self, tmp_path):
        """切换到 oc 后，omo_plugin.txt 应包含被移除的条目"""
    ...
    def test_already_oc_mode_no_error(self, tmp_path):
        """已是 oc 模式时再次切换不应报错"""
...
class TestSwitchToOmo:
    def test_restores_omo_entry(self, tmp_path):
        """oc → omo round-trip：切换回 omo 后 plugin 列表应包含原始条目"""
        switcher, config_path = make_switcher(
            tmp_path, ["oh-my-opencode@3.7.4"]
        )
        switcher.switch_to_oc()
    ...
    def test_no_txt_file_mode_stays_oc(self, tmp_path):
        """omo_plugin.txt 不存在时，模式应保持 oc"""
        switcher, _ = make_switcher(tmp_path, [])
        switcher.switch_to_omo()
        assert switcher.get_current_mode() == "oc"
...
class TestGetCurrentMode:
    ...
    def test_returns_oc_when_no_omo(self, tmp_path):
        """plugin 列表不含 oh-my-opencode 时返回 'oc'"""
        switcher, _ = make_switcher(tmp_path, ["other-plugin@1.0.0"])
        assert switcher.get_current_mode() == "oc"

    def test_returns_oc_when_empty_list(self, tmp_path):
        """plugin 列表为空时返回 'oc'"""
        switcher, _ = make_switcher(tmp_path, [])
        assert switcher.get_current_mode() == "oc"

    def test_only_returns_oc_or_omo(self, tmp_path):
        """返回值只能是 'oc' 或 'omo'"""
        for plugins in [[], ["oh-my-opencode@1.0"], ["other@1.0"]]:
            switcher, _ = make_switcher(tmp_path, plugins)
            assert switcher.get_current_mode() in ("oc", "omo")
